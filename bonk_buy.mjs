...
const wallet = new Wallet(Keypair.fromSecretKey(
  Uint8Array.from(
    JSON.parse(
      fs.readFileSync(
        process.env.PRIVATE_KEY,  // this would be a key as a 32 byte array 
        'utf-8',
      ),
    ),
  )
...
// --- USER INPUTS --- //
// baseAmount = amount of SOL you want to spend (in lamports = 10^9 SOL)
const baseAmount = 100_000_000; //0.1 SOL
const SLIPPAGE_MAX = 0.2;  // as a % = 0.1, 0.2, 0.5, 1, 2, ...
const PRIORITY_RATE = 100; // lamports per compute unit
const DECIMALS = 1E5;

console.log("attempting to buy " + baseAmount / LAMPORTS_PER_SOL + " SOL worth of BONK");
console.log("priority fee is set to " + PRIORITY_RATE + " micro-lamports per compute unit (10^-15 SOL/CU.");

const solToBonk = await getCoinQuote(SOL_MINT, BONK_MINT, baseAmount, SLIPPAGE_MAX);

const quoteAmount = solToBonk.data[0].outAmount
console.log(`you will receive ${quoteAmount / DECIMALS} bonk`);

await Promise.all(
  [solToBonk.data[0]].map(async (route) => {
